...
export function registerChatbotRoutes(app) {
    ...
    app.get('/api/chat/recommendations', async (req, res) => {
        try {
            ...
            if (location) {
                filteredHotels = hotels.filter(h => h.location.toLowerCase().includes(String(location).toLowerCase()));
            }
...
async function generateLLMResponse(message, llm) {
    // Get context about available hotels
    const hotels = await hotelService.getAllHotels();
    const recentPosts = await postService.getAllPosts();
    const hotelContext = hotels.slice(0, 5).map(h => `${h.name} in ${h.location}${h.rating ? ` (${h.rating}/5)` : ''}`).join(', ');
...
async function generateRuleBasedResponse(message) {
